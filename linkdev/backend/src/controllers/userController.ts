...
import { AuthRequest, UserFilters } from '../types/index.js';
import { createError, asyncHandler } from '../middleware/errorHandler.js';

const convertUserFromDb = (dbUser: any) => ({
  id: dbUser.id,
  email: dbUser.email,
...
export const searchUsers = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { search, role, page = 1, limit = 10 }: UserFilters = req.query;
  ...
  const result = await pool.query(
    `SELECT id, email, first_name, last_name, headline, about, profile_picture, 
...
export const getUserPosts = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const result = await pool.query(
    `SELECT p.*, 
     ...
     LIMIT $2 OFFSET $3`,
    req.user ? [...queryParams, req.user.id] : queryParams
  );
...
export const getRecommendedUsers = asyncHandler(async (req: AuthRequest, res: Response) => {
  if (!req.user) {
  ...
  const result = await pool.query(
    `SELECT DISTINCT u.id, u.first_name, u.last_name, u.headline, u.profile_picture, 
