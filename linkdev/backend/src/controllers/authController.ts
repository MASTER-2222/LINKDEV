...
import { AuthRequest, LoginData, RegisterData, User } from '../types/index.js';
...
const convertUserFromDb = (dbUser: any): User => ({
  id: dbUser.id,
  email: dbUser.email,
...
export const uploadCoverPhoto = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const result = await pool.query(
    'UPDATE users SET cover_photo = $1 WHERE id = $2 RETURNING cover_photo',
    [imageUrl, req.user.id]
  );
