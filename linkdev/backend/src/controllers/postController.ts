...
const convertPostFromDb = (dbPost: any) => ({
  id: dbPost.id,
...
const convertCommentFromDb = (dbComment: any) => ({
  id: dbComment.id,
...
export const createPost = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const result = await pool.query(
    `INSERT INTO posts (content, author_id, image_url) 
     VALUES ($1, $2, $3) 
     RETURNING *`,
    [content.trim(), req.user.id, imageUrl]
  );
...
export const getPostComments = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { postId } = req.params;
...
export const createComment = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const result = await pool.query(
    `INSERT INTO comments (content, author_id, post_id) 
     VALUES ($1, $2, $3) 
     RETURNING *`,
    [content.trim(), req.user.id, postId]
  );
