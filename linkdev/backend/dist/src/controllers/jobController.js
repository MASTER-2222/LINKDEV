...
const convertApplicationFromDb = (dbApp) => ({
    id: dbApp.id,
    jobId: dbApp.job_id,
...
export const getJobs = asyncHandler(async (req, res) => {
    ...
    // Count total jobs
    const countResult = await pool.query(`SELECT COUNT(*) FROM jobs j WHERE ${whereConditions.join(' AND ')}`, queryParams);
...
export const createJob = asyncHandler(async (req, res) => {
    ...
    const { title, company, location, description, requirements, salary, jobType, workMode, experienceLevel, } = req.body;
    // Validation
    ...
    RETURNING *`, [title, company, location, description, requirements, salary, jobType, workMode, experienceLevel, req.user.id]);
...
export const updateJob = asyncHandler(async (req, res) => {
    ...
    const { title, company, location, description, requirements, salary, jobType, workMode, experienceLevel, isActive, } = req.body;
    const updates = [];
    const values = [];
    let paramCount = 1;
    if (title !== undefined) {
    ...
    values.push(jobId);
    const result = await pool.query(`UPDATE jobs SET ${updates.join(', ')} WHERE id = $${paramCount} RETURNING *`, values);
...
export const getMyJobs = asyncHandler(async (req, res) => {
    ...
    const result = await pool.query('SELECT * FROM jobs WHERE recruiter_id = $1 ORDER BY created_at DESC', [req.user.id]);
...
export const applyToJob = asyncHandler(async (req, res) => {
    ...
    const jobResult = await pool.query('SELECT * FROM jobs WHERE id = $1 AND is_active = true', [jobId]);
    if (jobResult.rows.length === 0) {
        throw createError(404, 'Job not found or not active');
    }
    // Check if user already applied
    const existingApplication = await pool.query('SELECT id FROM job_applications WHERE job_id = $1 AND applicant_id = $2', [jobId, req.user.id]);
    if (existingApplication.rows.length > 0) {
        throw createError(409, 'You have already applied to this job');
    }
    const result = await pool.query(`INSERT INTO job_applications (job_id, applicant_id, cover_letter) 
     VALUES ($1, $2, $3) 
     RETURNING *`, [jobId, req.user.id, coverLetter]);
...
export const getMyApplications = asyncHandler(async (req, res) => {
    ...
    const result = await pool.query(`SELECT ja.*, 
...
export const updateApplicationStatus = asyncHandler(async (req, res) => {
    ...
    const result = await pool.query(`SELECT ja.*, j.recruiter_id 
     FROM job_applications ja
     JOIN jobs j ON ja.job_id = j.id
    ...
    const updateResult = await pool.query('UPDATE job_applications SET status = $1 WHERE id = $2 RETURNING *', [status, applicationId]);
